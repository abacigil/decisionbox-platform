...
func getPromptsPath() string {
	if p := os.Getenv("DOMAIN_PACK_PATH"); p != "" {
		return filepath.Join(p, "travel", "prompts")
	}
	return "domain-packs/travel/prompts"
}

// getProfilesPath returns the path to profiles, checking env var on each call.
func getProfilesPath() string {
	if p := os.Getenv("DOMAIN_PACK_PATH"); p != "" {
		return filepath.Join(p, "travel", "profiles")
	}
	return "domain-packs/travel/profiles"
}

// Compile-time check: TravelPack implements DiscoveryPack.
var _ domainpack.DiscoveryPack = (*TravelPack)(nil)
...
// DomainCategories returns the travel sub-domain categories.
func (p *TravelPack) DomainCategories() []domainpack.DomainCategory {
	return []domainpack.DomainCategory{
		{
			ID:          "vacation_rental",
			Name:        "Vacation Rental",
			Description: "Vacation rental marketplaces with property listings, host management, and guest bookings (e.g., Airbnb, VRBO, Booking.com)",
		},
	}
}

// AnalysisAreas returns base + category-specific analysis areas.
func (p *TravelPack) AnalysisAreas(categoryID string) []domainpack.AnalysisArea {
...
func (p *TravelPack) Prompts(categoryID string) domainpack.PromptTemplates {
...
func (p *TravelPack) ProfileSchema(categoryID string) map[string]interface{} {
